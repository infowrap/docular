...
var generate = function (options_in, callBack) {
    ...
    var githubUrl = options.githubUrl || false;
    ...
    var generateConfigScript = function (writeFutures, baseUrl) {
        ...
        if(options.githubUrl) {
            // Add fork me bar when a githubUrl is available
            configScript += "window.githubUrl = '" + options.githubUrl + "';";
        }
    ...
    var generateWebappPartials = function (writeFutures) {
        ...
        var getPartialName = function (pName) {
            return pName;
        };
    ...
    Q.when(loadConfiguredPlugins(), function(content) {}).then(function(){
        ...
        return writer.makeDir(REL_WEBAPP + '/documentation/partials/', true);
    ...
    //create a directory to store controllers from plugins
    }).then(function(){

        return writer.makeDir(REL_WEBAPP + '/controller/', true);


    //create a directory to store configs for the plugins
    }).then(function(){

        return writer.makeDir(REL_WEBAPP + '/configs/', true);
    ...
    // we need to copy over all UI resources from the doc apis and generate the index.html page with the doc_api resources injected
        ...
        for(var api in doc_apis) {
            if (doc_apis.hasOwnProperty(api)){
                ...
                for(i=0; i < doc_apis[api].ui_resources.css.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/' + docularAPIPrefix + api + '/' + doc_apis[api].ui_resources.css[i];
                ...
                for(i=0; i < doc_apis[api].ui_resources.js.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/' + docularAPIPrefix + api + '/' + doc_apis[api].ui_resources.js[i];
                    filePieces = sourceFileName.split('/');
                    targetFileName = filePieces[filePieces.length-1];
                    uiResourceList.push({
                        type:'js',
        ...
        for(var plugin in ui_plugins) {
            if (ui_plugins.hasOwnProperty(plugin)){
                ...
                for(i=0; i < ui_plugins[plugin].ui_resources.css.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/docular-plugin-' + plugin + '/' + ui_plugins[plugin].ui_resources.css[i];
                    filePieces = sourceFileName.split('/');
                    targetFileName = filePieces[filePieces.length-1];
                    uiResourceList.push({
                        type:'css',
                ...
                for(i=0; i < ui_plugins[plugin].ui_resources.js.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/docular-plugin-' + plugin + '/' + ui_plugins[plugin].ui_resources.js[i];
                    filePieces = sourceFileName.split('/');
                    targetFileName = filePieces[filePieces.length-1];
                    uiResourceList.push({
                        type:'js',
                ...
                if(ui_plugins[plugin].controllers){
                    ...
                    for(var controllerName in ui_plugins[plugin].controllers){
                        ...
                        var cPromise =  writer.makeDir(jsControllerDir, true).then(function(){

                            return writer.makeDir(phpControllerDir, true);

                        }).then(function(){

                            //copy the node controller
                            if(controller.node){
                                var nodeSource = ABS_NODE_MODULES + '/docular-plugin-' + plugin + '/' + controller.node;
                            ...
                            if(controller.php){
                                var phpSource = ABS_NODE_MODULES + '/docular-plugin-' + plugin + '/' + controller.php;
        ...
        var jsServerConfigsPromise = writer.makeDir(REL_LIB + '/nodeserver/configs/', true).then(function(){
            return writer.output(jsServerConfigsTarget,["exports.configs=" + JSON.stringify(serverConfigs)]);
        });
        ...
        var phpServerConfigsPromise = writer.makeDir(REL_WEBAPP + '/configs/', true).then(function(){
            return writer.output(phpServerConfigsTarget,["<?php $configs= json_decode('" + JSON.stringify(serverConfigs) + "', true); ?>"]);
        });
        ...
        var phpControllerListPromise = writer.makeDir(REL_WEBAPP + '/controller/').then(function(){
            return writer.output(phpControllerListTarget, ["<?php $controllerList = json_decode('" + JSON.stringify(controllerList) + "'); ?>"]);
        });
        ...
        var clientConfigsPromise = writer.makeDir(REL_WEBAPP + '/documentation/', true).then(function(){
            return writer.output(clientConfigsTarget,['DATA_CONFIGS=' + JSON.stringify(clientConfigs)]);
        });
    ...
    //lastly lets just generate a nice report
    }).then(function() {
