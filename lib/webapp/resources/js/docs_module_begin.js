...
docsApp.directive.backToTop = function($timeout, $window) {
    return {
        restrict: 'A',
        link: function(scope, element, attrs) {
            element.bind('click', function() {
            ...
            // helps with initialization of view
            $timeout(function(){
                resizeDocs();
            }, 50);
...
docsApp.directive.forkMeBar = function() {
    return {
        restrict: 'A',
        replace:true,
        scope:{},
        template:[
            ...
            '<img style="position: absolute; top: 0; right: 0; border: 0;  z-index:999999;" src="https://s3.amazonaws.com/github/ribbons/forkme_right_white_ffffff.png" alt="Fork me on GitHub">',
            '</a>'
        ].join(''),
        link: function(scope, element, attrs) {
            if(window.githubUrl) {
                // only show bar when githubUrl is defined
                scope.githubUrl = window.githubUrl;
            } else {
                // otherwise remove it
                element.remove();
            }
